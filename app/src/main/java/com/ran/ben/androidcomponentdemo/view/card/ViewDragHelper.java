...
import com.ran.ben.androidcomponentdemo.view.EaseCubicInterpolator;
...
public class ViewDragHelper {
    ...
    private static final String TAG = "ViewDragHelp";
    ...
    private ScrollerCompat mAnticipateOvershootScroller;
    private ScrollerCompat mOvershootScroller;
    private ScrollerCompat mScrollerAccelerate;
    ...
    private ViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        ...
        mOvershootScroller = ScrollerCompat.create(context,
                new EaseCubicInterpolator(0.52f,1.47f,0.74f,1.2f));
        mAnticipateOvershootScroller = ScrollerCompat.create(context, new AnticipateInterpolator(0.7f));
        mScrollerAccelerate = ScrollerCompat.create(context, new AccelerateInterpolator());
        mScroller = mOvershootScroller;
    }
    ...
    public boolean smoothSlideViewTo(View child, int finalLeft, int finalTop) {
        ...
        mScroller = mScrollerAccelerate;
        return forceSettleCapturedViewAt(finalLeft, finalTop, 0, 0);
    }

    public boolean smoothSlideViewTo(View child, int finalLeft, int finalTop, int xvel, int yvel) {
        mCapturedView = child;
        mActivePointerId = INVALID_POINTER;

        mScroller = mScrollerAccelerate;
        return forceSettleCapturedViewAt(finalLeft, finalTop, xvel, yvel);
    }

    public boolean smoothSlideViewToAnticipateOvershoot(View child, int finalLeft, int finalTop) {
        mCapturedView = child;
        mActivePointerId = INVALID_POINTER;

        mScroller = mAnticipateOvershootScroller;
        return forceSettleCapturedViewAt(finalLeft, finalTop, 0, 0);
    }

    public boolean smoothSlideViewToOvershoot(View child, int finalLeft, int finalTop) {
        mCapturedView = child;
        mActivePointerId = INVALID_POINTER;

        mScroller = mOvershootScroller;
        return forceSettleCapturedViewAtOvershoot(finalLeft, finalTop, 0, 0);
    }
    ...
    private boolean forceSettleCapturedViewAt(int finalLeft, int finalTop, int xvel, int yvel) {
        ...
        if (CardSlidePanel.DEBUG) {
            Log.d(TAG, "forceSettleCapturedViewAt: duration = " + duration);
        }
    ...
    private boolean forceSettleCapturedViewAtOvershoot(int finalLeft, int finalTop, int xvel, int yvel) {
        final int startLeft = mCapturedView.getLeft();
        final int startTop = mCapturedView.getTop();
        final int dx = finalLeft - startLeft;
        final int dy = finalTop - startTop;
        ...
        mScroller.startScroll(startLeft, startTop, dx, dy, duration + 150);
